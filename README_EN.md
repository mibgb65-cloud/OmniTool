...
| Base64 codec | Encode and decode Base64 text locally, including multi-byte characters, with no install or network calls |
...
1. Choose **Base64 codec** from the sidebar or the mobile tab bar.
2. Pick **Encode** or **Decode**, then type or paste text to see the result update live.
3. **Switch direction** carries the current result into the input and reverses the mode; **Copy result** copies it to the clipboard.

Base64 is an encoding, not encryption: it needs no key and anyone can reverse it, so do not treat it as a substitute for encryption. Decoding tolerates missing padding and pasted text with line breaks. Multi-byte characters such as Chinese are handled as UTF-8.
...
- A use is counted after a successful quick generation, when a saved account code is copied, or when a Base64 result is copied. Automatic 30-second refreshes are not counted.
...
- Node.js built-in test runner for RFC, Base64, and Worker behavior tests
...
| `npm test` | Run the TOTP, Base64, and Worker unit tests |
